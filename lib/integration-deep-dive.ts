 ...
 * ! benchmarks — "under three kilobytes, loaded async" is the honest weight
...
export const integrationDeepDives: Record<string, IntegrationDeepDive> = {
  ...
  svelte: {
    ...
    paragraphs: [
      'Svelte and SvelteKit are built around shipping as little JavaScript as possible, and a three-kilobyte async analytics script honours that budget. Drop it into `app.html` and it tracks SvelteKit’s client-side navigations without a store or a lifecycle hook — it never touches your compiled component output, so your bundle stays lean.',
  ...
  laravel: {
    ...
    paragraphs: [
      'Laravel serves rendered HTML from Blade, so analytics belongs in the layout, not in a client bundle. Add the Pulse script once to your main Blade layout and every server-rendered page carries it — under three kilobytes, loaded async, with no impact on your response time and no client-side state to manage.',
  ...
  astro: {
    ...
    paragraphs: [
      'Astro ships zero JavaScript by default and renders mostly static HTML, so a heavyweight analytics tag would be badly out of place. Pulse’s sub-three-kilobyte async script is about the only client-side JavaScript a content site needs, and because Astro output is static and cacheable, there is no per-visitor cookie to break your CDN caching.',
