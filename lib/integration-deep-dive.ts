 ...
 * ! benchmarks — "about five kilobytes, loaded async" is the honest weight
...
export const integrationDeepDives: Record<string, IntegrationDeepDive> = {
  ...
  react: {
    ...
    paragraphs: [
      'A React SPA — Create React App, Vite, or any client-rendered setup — already ships a meaningful JavaScript bundle, so every extra kilobyte counts. Pulse’s script is about five kilobytes loaded from `index.html`, entirely outside your component tree, and it records SPA route changes through the History API without you writing a `useEffect` for it. There is no analytics provider to thread through context and no re-render cost.',
  ...
  svelte: {
    ...
    paragraphs: [
      'Svelte and SvelteKit are built around shipping as little JavaScript as possible, and a five-kilobyte async analytics script honours that budget. Drop it into `app.html` and it tracks SvelteKit’s client-side navigations without a store or a lifecycle hook — it never touches your compiled component output, so your bundle stays lean.',
  ...
  laravel: {
    ...
    paragraphs: [
      'Laravel serves rendered HTML from Blade, so analytics belongs in the layout, not in a client bundle. Add the Pulse script once to your main Blade layout and every server-rendered page carries it — about five kilobytes, loaded async, with no impact on your response time and no client-side state to manage.',
