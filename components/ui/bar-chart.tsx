...
function TooltipBox({
  ...
  return createPortal(
    <motion.div animate={{ opacity: 1, scale: 1, y: 0 }} className={cn("pointer-events-none absolute z-50", className)} exit={{ opacity: 0, scale: 0.92, y: 4 }} initial={{ opacity: 0, scale: 0.92, y: 4 }} ref={tooltipRef} style={{ left: animatedLeft, top: finalTop }} transition={{ duration: DURATION_FAST, ease: EASE_APPLE }}>
      <motion.div animate={{ scale: 1, opacity: 1, x: 0 }} className="min-w-[140px] overflow-hidden rounded-none bg-popover border border-border text-white" initial={{ scale: 0.85, opacity: 0, x: shouldFlipX ? 20 : -20 }} key={flipKey} style={{ transformOrigin }} transition={SPRING}>
        {children}
      </motion.div>
    </motion.div>,
    container
