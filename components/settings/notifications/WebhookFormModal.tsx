...
import { Button, Input, Checkbox, Modal } from '@ciphera-net/facet'
...
export default function WebhookFormModal({ onClose, onCreated }: Props) {
  ...
  return (
    <Modal isOpen onClose={onClose} title="Add webhook" className="sm:max-w-[480px]">
      <div className="space-y-4 text-sm">
        <div>
          ...
          />
          {fieldErrors.url && <p className="mt-1 text-xs text-destructive">{fieldErrors.url}</p>}
        </div>
        ...
        <div>
          <label className="mb-2 block text-xs font-medium text-muted-foreground">Subscribed event types</label>
          ...
          {fieldErrors.types && <p className="mt-1 text-xs text-destructive">{fieldErrors.types}</p>}
        </div>
